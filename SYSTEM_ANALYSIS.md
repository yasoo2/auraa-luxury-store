...
- **لم تُربط** `super_admin.py` و`super_admin_simple.py` و`admin_products_routes.py`: مساراتها لا
  ...
  نقاط إدارة المستخدمين التي تستدعيها الواجهة فعلاً.
- **لم تُنفَّذ** بقية سطح الإدارة (CMS، الوسائط، الثيم، التحليلات، تكامل AliExpress) — نحو 20 نقطة
  تستدعيها الواجهة ولا وجود لها في أي ملف. هذه **ميزات جديدة** تحتاج قرارات منتج، لا إصلاح أعطال.
